...
class TestApiGraph:
    ...
    # ── folder colour-coding tests ───────────────────────────────────────────

    def _note_elements(self, data):
        """Elements that are real note nodes (no edges)."""
        return [e for e in data["elements"] if "source" not in e["data"]]

    def test_node_has_folder_field(self, server):
        base, _ = server
        _, data = get_json(base + "/api/graph")
        parser_node = next(e["data"] for e in data["elements"]
                           if e["data"].get("id") == "modules/parser")
        assert parser_node["folder"] == "modules"

    def test_top_level_node_has_empty_folder(self, server):
        base, _ = server
        _, data = get_json(base + "/api/graph")
    ...
    def test_node_label_is_full_slug(self, server):
        base, _ = server
        _, data = get_json(base + "/api/graph")
        parser_node = next(e["data"] for e in data["elements"]
                           if e["data"].get("id") == "modules/parser")
        assert parser_node["label"] == "modules/parser"

    def test_no_compound_folder_nodes(self, server):
        """No synthetic folder nodes should appear in the elements list."""
        base, _ = server
        _, data = get_json(base + "/api/graph")
    ...
    def test_path_slug_has_no_parent_field(self, server):
        base, _ = server
        _, data = get_json(base + "/api/graph")
        parser_node = next(e["data"] for e in data["elements"]
                           if e["data"].get("id") == "modules/parser")
        assert "parent" not in parser_node

    def test_deep_path_slug_folder(self, tmp_path):
        """a/b/c note → folder is 'a/b', not just 'a'."""
        s = EshpStore(tmp_path)
        s.upsert_note(_note("a/b/c", desc="Deep note"))
        s.conn.commit()
        http = make_server(tmp_path, host="127.0.0.1", port=0)
        port = http.server_address[1]
        t = threading.Thread(target=http.serve_forever, daemon=True)
        ...
        try:
            _, data = get_json(f"http://127.0.0.1:{port}/api/graph")
            node = next(e["data"] for e in data["elements"]
                        if e["data"]["id"] == "a/b/c")
            assert node["folder"] == "a/b"
        finally:
            http.shutdown()
            s.close()
